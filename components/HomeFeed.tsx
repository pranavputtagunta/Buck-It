...
interface EventItem  { 
  id: string; user: string; locationName: string; 
  coords: { latitude: number; longitude: number }; 
  title: string; image: string; deadline: string; 
  bucketItemId: string; tags: string[]; time_slots: string[]; 
}
interface PostItem   { 
  id: string; user: string; locationName: string; caption: string; 
  image: string; likes: number; tags: string[]; category: string; 
}
...
export default function HomeFeed() {
  ...
  useEffect(() => {
    const buildFeed = async () => {
      ...
      try {
        ...
        const { data: profile } = await supabase.from('users').select('display_name').eq('id', uid).single();
        ...
        if (postsData) setDbPosts(postsData.map(p => ({
          id: p.id, user: p.user_name, locationName: p.location_name, caption: p.caption,
          image: p.image, likes: p.likes, tags: p.tags, category: p.category
        })));

        // 4. Fetch & Rank Events
        const { data: globalEvents } = await supabase.from('discover_events').select('*');
        if (globalEvents) {
          const formatted: EventItem[] = globalEvents.map(ev => ({
            id: ev.id, user: 'Bucket Community', locationName: ev.location_name,
            coords: { latitude: ev.latitude || 32.7157, longitude: ev.longitude || -117.1611 },
            title: ev.title, image: ev.image, deadline: ev.deadline,
            bucketItemId: ev.bucket_item_id, tags: ev.tags || [], time_slots: ev.time_slots || [],
      ...
      } catch (err) { console.error("Feed build error:", err); } finally { setIsLoadingFeed(false); }
    };
    buildFeed();
  }, []);
  ...
  const openShareModal = (item: any) => {
    ...
    Animated.spring(shareSlideAnim, { toValue: 0, useNativeDriver: true, bounciness: 0 }).start();
  };

  const closeShareModal = () => {
    Animated.timing(shareSlideAnim, { toValue: SCREEN_HEIGHT, duration: 200, useNativeDriver: true }).start(() => {
  ...
  const handleLockItIn = async (item: EventItem) => {
    ...
    try {
      ...
      await supabase.from('events').insert({ user_id: userId, title: item.title, status_text: 'Awaiting RSVPs', participants: participants.length, is_active: false, completed: false });
  ...
  const handleShareToChat = async (chatId: string) => {
    ...
    try {
      ...
      await supabase.from('chat_messages').insert({
        chat_id: chatId, user_id: userId, sender_name: userName, text: shareText, type: isAIGen ? 'ai' : 'user'
      });
      ...
      await fetch(`${apiBase}/api/chat/`, {
        ...
        body: JSON.stringify({ 
          chat_id: chatId, sender_name: userName, text: shareText,
          metadata: isAIGen ? { tags: selectedItem.tags, category: selectedItem.category, location: selectedItem.locationName } : null
        })
      });
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={styles.headerIcons}>
            {feedType === 'Events' && <TouchableOpacity style={styles.iconBtn} onPress={openFilter}><Filter color={activeFilterIds.length > 0 ? C.blue : C.textPrimary} size={24} /></TouchableOpacity>}
            <TouchableOpacity style={styles.iconBtn}><PlusSquare color={C.textPrimary} size={24} /></TouchableOpacity>
          ...
          <Pressable style={styles.overlay} onPress={() => setIsPostOptionsVisible(false)}>
            <View style={styles.optionSheet}>
              ...
              <TouchableOpacity style={styles.optionBtn} onPress={() => { setIsPostOptionsVisible(false); openShareModal(selectedItem); }}>
                <Send color={C.textPrimary} size={20} style={{ marginRight: 12 }} /><Text style={styles.optionBtnText}>Share as Post</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.optionBtn, { backgroundColor: C.accentLight }]} onPress={() => { setIsPostOptionsVisible(false); openShareModal({...selectedItem, mode: 'AI_GEN'}); }}>
                <Sparkles color={C.accentDark} size={20} style={{ marginRight: 12 }} /><Text style={[styles.optionBtnText, { color: C.accentDark }]}>Share as AI Event</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  eventTitle: { fontSize: 16, fontWeight: '700', color: C.textPrimary, marginBottom: 8, flex: 1 },
  ...
  likesText: { fontWeight: '700', color: C.textPrimary, marginBottom: 4, marginTop: 6, paddingHorizontal: 14 },
  captionText: { color: C.textPrimary, fontSize: 14, lineHeight: 20, paddingHorizontal: 14 },
  ...
  optionSheet: { backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 },
  ...
  sheetTitle: { fontSize: 17, fontWeight: '700', color: C.textPrimary, textAlign: 'center', marginBottom: 20 },
  optionBtn: { flexDirection: 'row', alignItems: 'center', padding: 18, borderRadius: 16, borderWidth: 1, borderColor: C.border, marginBottom: 12 },
  ...
  chatRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderColor: C.bg },
  chatAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: C.accentLight, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
