...
export default function HomeFeed() {
  ...
  useEffect(() => {
    const buildFeed = async () => {
      ...
      try {
        ...
        // Smash it all together into one giant searchable string
        const userPersonaQuery = [personality, ...interests, ...bucketTags, ...slots].filter(Boolean).join(" ");
        ...
        if (eventsData) {
          ...
          // 5. Algorithm Sort: Use Fuse to rank events against the user's Persona Query
  ...
  const openFilter = () => { setIsFilterVisible(true); Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, bounciness: 0, speed: 12 }).start(); };
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        ...
        {isLoadingFeed ? (
          ...
          <FlatList
            data={activeFilterIds.length ? recommendedEvents.filter((e) => activeFilterIds.includes(e.bucketItemId)) : recommendedEvents}
