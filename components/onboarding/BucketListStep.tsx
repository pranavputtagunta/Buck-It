...
import { Calendar, Trash2, Sparkles, Plus, ArrowRight } from 'lucide-react-native';
import { C } from './theme';
...
export default function BucketListStep({
  bucketList, onGenerateWithAI, onAddManual,
  onUpdateBucket, onRemoveBucket, onContinue, generating,
}: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Draft your first few goals, or let AI suggest some based on your vibe.</Text>
        ...
        {onGenerateWithAI && (
          ...
          >
            {generating
              ...
              : <>
                  <Sparkles color={C.accentDark} size={16} style={{ marginRight: 6 }} />
        ...
        <View key={i} style={styles.card}>
          ...
          {Platform.OS === 'web' ? (
            ...
            >
              ...
              <Text style={{ color: item.deadline ? C.textPrimary : C.textLight, fontSize: 14, fontWeight: '500' }}>
      ...
      {activePicker !== null && Platform.OS !== 'web' && (
        <DateTimePicker
          ...
          onChange={(e: any, d?: Date) => {
            ...
            if (d && e.type === 'set') onUpdateBucket(activePicker, 'deadline', d.toISOString().split('T')[0]);
          }}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  subtitle:{ fontSize: 15, color: C.textMuted, marginTop: 8, marginBottom: 24, lineHeight: 22 },

  // Action buttons
  actionRow:    { flexDirection: 'row', gap: 10, marginBottom: 20 },
  actionBtn:    { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 13, borderRadius: 12 },
  aiBtn:        { backgroundColor: C.accentLight, borderWidth: 1, borderColor: C.borderMid },
  ...
  cardIndex: { fontSize: 10, fontWeight: '700', color: C.textLight, letterSpacing: 1 },
  removeBtn: {
  ...
  fieldWrap: {
    ...
    borderWidth: 1, borderColor: C.border,
    paddingHorizontal: 12, marginBottom: 8, height: 44,
